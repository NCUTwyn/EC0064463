...
BA = np.load('BA.npy')
ba= nx.random_graphs.barabasi_albert_graph(1000,4)  #nx.random_graphs.random_regular_graph(3,70)
theNET=ba
b=1.5
eps = 0.02
theta=1
...
def init_QT_NS(theNET):
    ...
    return QTable,NodeStates,UTable
...
def exploitation_action(theNET, NodeStates, QTable,UTable):
    ...
    for i in theNET.nodes():
        nbrs = UTable[str(i)].columns.values.tolist() #节点i的所有邻居.#nbrsi = theNET.neighbors(j)  错误原因：应该是theNET.neighbors(i) 应该是i的邻居，而不是j
        for j in nbrs:
            cij = 0
            for aj in c: #对于邻居j 的两个动作0，1
                ...
                #cij =( cij + UTable[str(j)].loc[aj, i])
            #UTable[str(j)].loc[aj, i]=UTable[str(j)].loc[aj, i]-cij/2
            #UTable[str(j)].loc[aj, i] = UTable[str(j)].loc[aj, i] - cij / 2
        gi = g_ai(i, UTable)  #
        if gi[0,0]==gi[0,1]:
...
def main_proc(theNET,eps,alpha,gamma):
    #Initialize 初始化参数
    (QTable, NodeStates,UTable) = init_QT_NS(theNET)
    ...
    return QTable, oldNodeStates_list, oldc_ratio,UTable
...
if __name__ == "__main__":
    ( QTable, oldNodeStates_list,oldc_ratio,UTable) =main_proc(theNET,eps,alpha,gamma)

print( QTable, oldNodeStates_list,oldc_ratio,UTable)
...
print(test)
test.to_csv('D:/111111111研三论文/更改之后的数据/Max-plus/Max-plus-图1-100次-1000个节点.csv')
